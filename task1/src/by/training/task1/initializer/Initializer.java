...
public class Initializer {
    ...
    /**
     * Static method to take {@code Car} instances
     * from the file and put them into the repository.
     * It calls {@code Parser.parseFromBuf} to parsed data from file,
     * so variable {@code wrongListAmount}
     * counts how mush lines are invalid to calculate identifier.
     *
     * @param filename input file
     * @param repository repository to put
     * @throws IOException in case mistake with opening the file
     */
    public void initializeFromFile(final String filename, final Repository<Car> repository) throws IOException {
        List<ArrayList<String>> data = new Parser().parseFromBuf(filename);
        int[] id = new int[]{1};
        for (ArrayList<String> params : data) {
            params.add(1, id[0] + "");
                new Creator().create(params).ifPresent(optionalCar -> {
